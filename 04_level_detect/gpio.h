...
inline void set_gpio_high_enable_detect(enum gpio pin)
{
    if ((pin >= gpio00) && (pin <= gpio31))
    {
        *GPHEN0 = set_bit_pattern(*GPHEN0, 1, pin, 1);
    }
    else
    {
        *GPEDS1 = set_bit_pattern(*GPHEN1, 1, pin-32, 1);
    }
}

inline int gpio_event_detected(enum gpio pin)
{
    enum { CLEAR_BIT=1, ONE_BIT=1 };
    if ((pin >= gpio00) && (pin <= gpio31))
    {
        unsigned int b = get_bit_pattern(*GPEDS0, pin, ONE_BIT);
        if (b == 1)
        {
            *GPEDS0 = set_bit_pattern(*GPEDS0, CLEAR_BIT, pin, ONE_BIT);
            return 1;
        }
        return 0;
    }
    ...
    {
        unsigned int b = get_bit_pattern(*GPEDS1, pin-32, ONE_BIT);
        if (b == 1)
        {
            *GPEDS1 = set_bit_pattern(*GPEDS1, CLEAR_BIT, pin-32, ONE_BIT);
            return 1;
        }
        return 0;
    }
